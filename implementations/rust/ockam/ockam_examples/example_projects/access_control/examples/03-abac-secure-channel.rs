use abac_examples::{
    AbacAuthorizationRequest, AbacAuthorizationResponse, AbacAuthorizationWorker, AbacPolicyWorker,
    AuthenticatedTableWorker,
};
...
async fn main(mut ctx: Context) -> Result<()> {
    ...
    let some_secure_channel = connect_to_ockam_cloud(&ctx).await?;
...
impl Worker for EchoerServiceWorker {
    ...
    async fn is_authorized(
        ...
        // Create a Resource, Action that represents whatever specific API
        // call you need to authorize
...
async fn connect_to_ockam_cloud(ctx: &Context) -> Result<Address> {
    // Start some ABAC policy source
    let mut backend = abac::mem::Memory::new();
    backend.populate_with_policy_test_data().await?;
    ...
    let mut backend = abac::mem::Memory::new();
    backend.populate_with_subject_test_data().await?;
