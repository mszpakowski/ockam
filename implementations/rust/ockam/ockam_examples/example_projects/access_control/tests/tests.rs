...
#[test]
#[serial] // Serialise tests that may clash on TCP ports
fn run_02_abac_for_transport() -> Result<(), Error> {
    // Launch responder, wait for it to start up
    let resp = CmdBuilder::new("cargo run --example 02-abac-for-transport-responder").spawn()?;
    resp.match_stdout(r"(?i)Waiting for incoming TCP connection")?;

    // Run initiator to completion
    let (exitcode, stdout) =
        CmdBuilder::new("cargo run --example 02-abac-for-transport-initiator").run()?;
...
#[test]
fn run_03_abac_secure_channel() -> Result<(), Error> {
    // Run to completion
    let (exitcode, stdout) = CmdBuilder::new("cargo run --example 03-abac-secure-channel").run()?;
    assert_eq!(Some(0), exitcode);
